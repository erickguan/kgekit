...
class LCWANoThrowSamplerTest(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.entity_sampler = kgekit.LCWANoThrowSampler(cls.triple_indexes, 2, 0)
        cls.relation_sampler = kgekit.LCWANoThrowSampler(cls.triple_indexes, 0, 1)
    ...
    def test_entity_sample(self):
        ...
        self.entity_sampler.sample(samples, [[True for _ in triple_indexes], triple_indexes], 100)
    ...
    def test_relation_sample(self):
        ...
        samples = np.zeros((batch_size, samples_per_batch, 3), dtype=np.int32)
        triple_indexes = self.triple_indexes[:batch_size]
        self.relation_sampler.sample(samples, [[True for _ in triple_indexes], triple_indexes], 100)
        np.testing.assert_array_equal(samples, np.array([
            [
                [0, 1, 3], [0, 3, 3]
            ]
